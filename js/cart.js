$(function () {
  // 第一个功能：先读取本地数据中的数据，然后动态生成列表结构
  let arr = kits.loadData('cartListData');
  // 遍历数组，生成指定结构
  // 先准备一个空字符串，用来存储生成动态结构的字符串
  let html = '';
  arr.forEach(e => {
    // 需要有一个产品的id，用于后期操作
  ...
  // 点选-所有的点选的checkbox都是动态生成，推荐使用委托离开实现
  $('.item-list').on('click', '.item-ck', function () {
    ...
    // 需要根据点选的商品的id，到本地数据中，修改ischecked属性
    let pID = $(this).parents('.item').attr('data-id');
    // 获取当前这个单选是否选中
    let isChecked = $(this).prop('checked');
    arr.forEach(e => {
      if (e.pID == pID) {
        // 就需要把当前这个产品的选中状态改成和勾选状态一样
  ...
  $('.item-list').on('click', '.item-del', function () {
    layer.confirm('你确定要要删除吗?', { icon: 0, title: '警告' }, (index) => {
      layer.close(index);
      // 在这里执行删除的逻辑
      // 先得到要删除的数据的id
      let id = $(this).parents('.item').attr('data-id');
